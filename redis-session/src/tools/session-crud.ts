...
import { Session, MCPToolResult, SearchOptions, KEY_PATTERNS, CACHE_KEYS } from '../types.js';
...
export class SessionCRUD {
  ...
  async createSession(sessionId: string, metadata?: Record<string, any>, ttl?: number): Promise<MCPToolResult<Session>> {
    try {
  ...
  async getSession(sessionId: string): Promise<MCPToolResult<Session>> {
    try {
      const cacheKey = `${CACHE_KEYS.ACTIVE_SESSIONS}:${sessionId}`;

      // Check cache first
      const cached = this.redis.getCache<Session>(cacheKey);
      if (cached) {
        return {
          success: true,
          data: cached
  ...
  async listSessions(pattern?: string, limit?: number): Promise<MCPToolResult<Session[]>> {
    try {
  ...
  async updateSession(sessionId: string, updates: Record<string, any>): Promise<MCPToolResult<Session>> {
    try {
      const sessionKey = this.getSessionKey(sessionId);
      ...
      const existingSession: Session = JSON.parse(existingData);
  ...
  async deleteSession(sessionId: string): Promise<MCPToolResult<boolean>> {
    try {
      const sessionKey = this.getSessionKey(sessionId);
      const indexKey = this.getSessionIndexKey();
  ...
  async getActiveSessions(): Promise<MCPToolResult<Session[]>> {
    try {
  ...
  async searchSessions(query: string, limit?: number): Promise<MCPToolResult<Session[]>> {
    try {
  ...
  async getSessionStats(): Promise<MCPToolResult<{ total: number; active: number; expired: number }>> {
    try {
      const indexKey = this.getSessionIndexKey();
  ...
  async cleanupExpiredSessions(): Promise<MCPToolResult<number>> {
    try {
      const indexKey = this.getSessionIndexKey();
